...
// 新增需求进展接口
void add_requirement_record(const Request& req, Response& res) {
    try {
...
// 修改需求进展接口
void update_requirement_record(const Request& req, Response& res) {
    try {
        auto j = json::parse(req.body);
        RequirementRecord record;
...
// 新建需求记录
void create_requirement(const Request& req, Response& res) {
    try {
        auto j = json::parse(req.body);
        RequirementRecord record;
        record.title = j.value("title", "");
